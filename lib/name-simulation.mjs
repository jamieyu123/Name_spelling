...
const SESSION_LANGUAGE_SPANISH = `Session language: Spanish. Write the confirmation in Spanish; spell names using Spanish letter names (nombre de letra: a, be, ce, erre, jota, etc.) — not English letter names or NATO. Between letter names use single spaces only, not commas (TTS often says "coma"); never include the word "coma" in the spelling.`;
...
function buildCustomerSystemPrompt(fullName, locale) {
  ...
  return [
    ...
    `If the agent asks you to spell letter by letter, spell your first name then your last name accurately, with spaces between letters — do not use hyphens or dashes between letters.`,
